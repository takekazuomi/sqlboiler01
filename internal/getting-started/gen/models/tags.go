...
var TagWhere = struct {
	...
	Deleted: whereHelperbool{field: "`tags`.`deleted`"},
}
...
func (r *tagR) GetVideos() VideoSlice {
	if r == nil {
		return nil
	}
	return r.Videos
...
var (
	tagAllColumns            = []string{"id", "name", "deleted"}
	tagColumnsWithoutDefault = []string{"name"}
	tagColumnsWithDefault    = []string{"id", "deleted"}
...
func (o *Tag) Videos(mods ...qm.QueryMod) videoQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`tags_videos` on `videos`.`id` = `tags_videos`.`video_id`"),
		qm.Where("`tags_videos`.`tag_id`=?", o.ID),
	)
...
func (tagL) LoadVideos(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTag interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("`videos`.`id`, `videos`.`user_id`, `videos`.`name`, `videos`.`deleted`, `a`.`tag_id`"),
		qm.From("`videos`"),
		qm.InnerJoin("`tags_videos` as `a` on `videos`.`id` = `a`.`video_id`"),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.UserID, &one.Name, &one.Deleted, &localJoinCol)
		if err != nil {
...
func (o *Tag) AddVideos(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Video) error {
	...
	for _, rel := range related {
		query := "insert into `tags_videos` (`tag_id`, `video_id`) values (?, ?)"
...
func (o *Tag) SetVideos(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Video) error {
	query := "delete from `tags_videos` where `tag_id` = ?"
...
func (o *Tag) RemoveVideos(ctx context.Context, exec boil.ContextExecutor, related ...*Video) error {
	...
	var err error
	query := fmt.Sprintf(
		"delete from `tags_videos` where `tag_id` = ? and `video_id` in (%s)",
