...
gorm/gorpのような「コードファースト」ではなく、「データベースファースト」なORMです。
...
つまり、最初にデータベーススキーマを作成する必要があります。データベースのライフサイクルのこの部分を管理するために、[sql-migrate](https://github.com/rubenv/sql-migrate)などの移行ツールを使ってください。
...
レガシーなRailsデータベースを移行しようとしたとき、開発速度という点で、ActiveRecordがいかに有益であるかに気づきました。
...
ActiveRecordを使った後に、Goの `database/sql` パッケージを使うと、非常に繰り返しが多く、超長文で、実に退屈に感じます。
...
Go のベテランである私たちは、ORM の状態が不安定であることを知っていましたが、簡単にレビューした結果、私たちが恐れていたことが確認されました。世の中のほとんどのパッケージは、コードファーストで、リフレクションベースで、モデル間のリレーションシップに関するストーリーが非常に弱いのです。そこで、私たちは次のような目標を掲げました。

* Work with existing databases: Don't be the tool to define the schema, that's better left to other tools.
* 既存のデータベースを使用します。スキーマを定義するツールにならないように、それは他のツールに任せた方が良い。
* ActiveRecord-like productivity: Eliminate all sql boilerplate, have relationships as a first-class concept.
* ActiveRecordのような生産性。すべてのSQL定型文を排除し、第一級の概念としてリレーションシップを持つ。
* Go-like feel: Work with normal structs, call functions, no hyper-magical struct tags, small interfaces.
* Go-like feel: 通常の構造体の操作、関数の呼び出し、超魔術的な構造体タグの排除、小さなインターフェイス。
* Go-like performance: [Benchmark](#benchmarks) and optimize the hot-paths, perform like hand-rolled `sql.DB` code.
* Go-like performance: [Benchmark](#benchmarks)でホットパスを最適化し、ハンドロールされた `sql.DB` コードのように実行します。
...
SQLBoilerとデータベースファーストのコード生成アプローチにより、これらの目標をすべて達成することができたと確信しています。さらに、SQLBoilerには次のようなメリットもあります。
...
* モデルパッケージはタイプセーフです。これは，間違った型を渡すことによるランダムなパニックの可能性がないことを意味します．interface{}は必要ありません．
...
* Goのデータ型は、データベースのカラム型と密接に関連しています。これは、ほぼすべてのGoデータ型をサポートする拡張Nullパッケージによって拡張されています。
...
* デバッグが容易なシステム。ORMはスキーマに合わせて作られており、コードパスはすべてreflectに埋もれていないため、簡単にトレースできるはずです。
* Auto-completion provides work-flow efficiency gains.
* オートコンプリートによるワークフローの効率化。
...
* SQLBoiler generates type safe identifiers for table names, table column names, a table's relationship names and type-safe where clauses. You should use these instead of strings due to the ability to catch more errors at compile time when your database schema changes. See [Constants](#constants) for details.
* SQLBoiler は、テーブル名、テーブルカラム名、テーブルのリレーション名、およびタイプセーフな Where 句のための型安全な識別子を生成します。文字列の代わりにこれらを使用すると、データベースのスキーマが変更されたときに、コンパイル時のエラーをより多く検出することができます。
 を使うべきです。詳しくは[Constants](#constants)を参照してください。
...
* sqlboiler が複数のデータベース呼び出しを行う場合 (たとえば挿入を伴うリレーションシップのセットアップ) は、パフォーマンスとデータの整合性の観点から、トランザクションを使用することを強くお勧めします。
...
* もし、フック機能を使う予定がなければ、`--no-hooks`フラグを使用してこの機能の生成を無効にすることができます。これにより、バイナリサイズを多少節約することができます。
...
プロジェクトに `sqlboiler` を依存関係としてインストールするには、以下のコマンドを go モジュールのディレクトリツリー内に記述してください。これにより、`go.mod` ファイルに依存関係が正しいバージョンでインストールされます。
...
When regenerating the models it's recommended that you completely delete the generated directory in a build script or use the `--wipe` flag in SQLBoiler.
モデルを再生成する際には、ビルドスクリプトで生成されたディレクトリを完全に削除するか、SQLBoilerの`--wipe`フラグを使用することをお勧めします。
...
その理由は、sqlboilerはファイルの差分を取ろうとせず、ファイルがあるかどうかにかかわらず、単に書き込もうとするファイルを書き込み、あなたや以前のSQLBoilerの実行によって追加されたファイルを削除しないためです。良い場合、これはコンパイルエラーの原因となり、悪い場合、データベースから削除されたテーブルに対して生成された、不要で使用不可能なコードが残る可能性があります。

The bottom line is that this tool should always produce the same result from the same source. And the intention is to always regenerate from a pure state.
要は、このツールは常に同じ源から同じ結果を出すべきだということです。そして、常に純粋な状態から再生することを意図しています。

The only reason the `--wipe` flag isn't defaulted to on is because we don't like programs that `rm -rf` things on the filesystem without being asked to.

wipe` フラグがデフォルトで on になっていないのは、頼まれてもいないのにファイルシステム上のものを `rm -rf` するようなプログラムが嫌いだからです。
...
テンプレートは毎回特定の方法で実行されます。コマンドライン/設定ファイルには様々な設定オプションがあり、どの機能をオンにするかオフにするかを制御することができます。
...
コマンドライン・フラグに加えて、設定ファイルを介してのみ利用可能ないくつかの機能があり、若干の説明を必要とします。

##### Aliases

In sqlboiler, names are automatically generated for you. If you name your database entities properly you will likely have descriptive names generated in the end. 

sqlboilerでは、名前は自動的に生成されます。データベースのエンティティに適切な名前を付けると、最終的に説明的な名前が生成される可能性が高いです。

However in the case where the names in your database are bad AND unchangeable, or sqlboiler's inference doesn't understand the names you do have (even though they are good and correct) you can use aliases to change the name of your tables, columns and relationships in the generated Go code.

しかし、データベースの名前が間違っていて変更できない場合や、sqlboilerの推論が名前を理解できない場合（正しい名前であっても）、エイリアスを使用して生成されるGoコードのテーブル、カラム、リレーションシップの名前を変更することができます。

*Note: It is not required to provide all parts of all names. Anything left out will be inferred as it was in the past.*

*注：すべての名称のすべての部分を記入する必要はありません。省略された部分は、過去と同様に推論されます。

```toml
# Although team_names works fine without configuration, we use it here for illustrative purposes
# team_names は設定しなくても問題なく動作しますが、ここでは説明のために使用します。
...
When creating aliases for relationships, it's important to know how sqlboiler names relationships. 

リレーションシップのエイリアスを作成する場合、sqlboilerがどのようにリレーションシップを命名するかを知っておくことが重要です。

For a given table the foreign key name is used as a unique identifier to refer to a given relationship.

与えられたテーブルに対して、外部キー名は、与えられた関係を参照するための一意な識別子として使用されます。

If you are going to be aliasing relationships it's **highly recommended** that you name your foreign keys explicitly in your database or the auto-generated names could one day change/break your aliases.

エイリアス関係を使用する場合、データベース内で外部キーに明示的に名前を付けることを **強く** お勧めします。

Each relationship has a **local** and a **foreign** function name.

各関係は、**local** と **foreign** の関数名を持っています。

The function name will be inserted into your generated code as a function to retrieve relationship data as well as refer to the relationship in a few other places.

この関数名は、リレーションシップデータを取得する関数として、生成されたコードに挿入されるだけでなく、他のいくつかの場所でもリレーションシップを参照します。

**local** means "the function name that refers to the table with the foreign key on it" and conversely **foreign** means "the function that refers to the table the foreign key points to".

**local** は「外部キーが乗っているテーブルを参照する関数名」、逆に **foreign** は「外部キーが指すテーブルを参照する関数」を意味します。
...
この例では、 `local` (外部キーを持つテーブルを参照する方法) は `Videos` と推論されることになります。これを上書きして、`AuthoredVideos` とします。
...
逆に `foreign` (外部キーが指すテーブルを参照する方法) は `User` と推論されますが、ドメイン言語に合うように `Author` にリネームしたいと思います。

With the configuration snippet below we can use the following relationship helper functions off of the respective models: `video.Author` and `user.AuthoredVideos` which make a bit more sense than the inferred ames when we see it in the code for our domain. Note the use of the foreign key name to refer to the relationship in the configuration key.

以下の設定スニペットで、それぞれのモデルから以下のリレーションヘルパー関数を使用することができます。`video.Author` と `user.AuthoredVideos` は、私たちのドメインのコードで見ると、推論されたエイムよりも少し理にかなっています。構成キーのリレーションシップを参照するために外部キー名を使用していることに注意してください。
...
# ローカル側はもともとAuthorVideosと推論されてしまうので、この機能をいじりたくない、可能な限り避けたいというのが本音でしょう。
...
# 指定しないままでも、外部キーカラムの適切な命名により、外部側が正しく推論されたはずである。
foreign = "Author"
```

In a many-to-many relationship it's a bit more complicated.

many-to-many relationshipでは、もう少し複雑です。

Let's look at an example relationship between `videos <-> tags` with a join table in the middle.

`videos <-> tags` のリレーションシップの例で、途中にジョインテーブルがある場合を見てみましょう。

Imagine if the join table didn't exist, and instead both of the id columns in the join table were slapped on to the tables themselves. 

もし、ジョインテーブルが存在せず、ジョインテーブルのidカラムが両方ともテーブル自体につけられていたらと想像してください。

You'd have `videos.tag_id` and `tags.video_id`.

videos.tag_id` と `tags.video_id` があるはずです。

Using a similar method to the above (local is the name with which we refer to the side that has the foreign key) we can rename the relationships.
To change `Videos.Tags` to `Videos.Rags` we can use the example below. 

上記と同様の方法（ローカルは外部キーを持つ側を参照する名前）を用いて、リレーションシップの名前を変更することができます。
以下の例では、`Videos.Tags` を `Videos.Rags` に変更することができます。
...
many-to-many relationship の片方に名前をつければ、もう片方は自動的にミラーリングされることを覚えておいてください。
...
上記の定義では、与えられた `Video` エンティティがすべてのタグにアクセスできるようにするためのプロパティ名として `Rags` を指定しています。逆に、一つの `Tag` エンティティは、 `Videos` プロパティを使用して、特定のタグを持つすべてのビデオを参照することができます。
